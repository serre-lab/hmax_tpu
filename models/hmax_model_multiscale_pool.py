...
from models import resnet_layers
...
def resnet_generator(block_fn,
  ...
  def model(inputs, is_training):
    ...
    c3 = tf.image.resize(c3, merge_size[1:3], align_corners=True, method=RESIZE_METHOD)
    c3 = tf.cast(c3, c2b.dtype)
    ...
    if pre_activation:
      s4 = norm_activation(s4, is_training, data_format=data_format,
                               layer=norm_act_layer, bn_momentum=bn_momentum)
    ...
    inputs = tf.reduce_mean(s4, reduction_indices=reduction_indices)
